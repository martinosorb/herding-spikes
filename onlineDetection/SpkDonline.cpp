...
void
Detection::InitDetection(long nFrames, double nSec, int sf, int NCh, long ti, long * Indices)
{
    NChannels = NCh;
    tInc      = ti;
...
void
Detection::SetInitialParams(int thres, int maa, int ahpthr, int maxsl, int minsl)
{
...
void
Detection::openSpikeFile(const char * name)
{
...
void
Detection::MedianVoltage(unsigned short * vm)  // easier to interpret, though it takes a little longer to run, but I'm sure there is a faster method in C++ for computing the median
...
void
Detection::MeanVoltage(unsigned short * vm) // if median takes too long... or there are only few channnels (?)
{
    int n;
    int Vsum;
...
void
Detection::Iterate(unsigned short * vm, long t0)
{
    ...
    for (int t = 0; t < tInc; t++) {          // loop over data, will be removed for an online algorithm
        ...
        for (int i = 0; i < NChannels; i++) { // loop across channels
            ...
            // DEFAULT OPERATIONS
                ...
                a = (vm[i * tInc + t] - Aglobal[t]) * Ascale - Qm[i]; // difference between ADC counts and Qm
                // UPDATE Qm and Qd
                if (a > 0) {
                    if (a > Qd[i]) {
                        Qm[i] += Qd[i] / Tau_m0;
                ...
                if (Sl[i] > 0) { // Sl frames after peak value
                    ...
                    if ((Sl[i] == MaxSl) & (AHP[i])) {
                        if ((2 * SpkArea[i]) > (MinSl * MinAvgAmp * Qd[i])) {
                            w << ChInd[i] << " " << t0 + t - MaxSl + 1 << " " << -Amp[i] * Ascale / Qd[i] << "\n";
                        }
                        Sl[i] = 0;
                    }
            ...
            // AFTER CHANNEL WAS OUT OF LINEAR REGIME
                ...
                Qm[i] = (2 * Qm[i] + (vm[i * NChannels + t] - Aglobal[t]) * Ascale + 2 * Qd[i]) / 3; // update Qm
                A[i]--;
            }
        }
    }
...
void
Detection::FinishDetection() // write spikes in interval after last recalibration; close file
{
    w.close();
}
}
