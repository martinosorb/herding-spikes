...
###This is the functions used in the program that writes the .txt File of the online detection
# into a .hdf File, and marks events detected in multiple channels and
# runs a correlation analysis.

### figure out dimensions of arrays, read spike files
def readSpikesFile(TxtFile, HdfFile, NoisyChFile, recCh=4096, removeCh=0, tMax=0, Sampling=7022):
	if recCh==4096:
		NCh=4096
	...
	f.create_dataset('RepolarizingSpikes', data=np.ones(np.sum(PreSelectedEvents),dtype=bool), dtype=bool)
...
def CorrelationAnalysis1(HdfFile, NextN=100, NPoissonNoise=120, dClockspikes=4, Nignore=20):
	f=h5py.File(HdfFile,'r')
	g=f['RawEvents']
	Sampling=f['Sampling'].value
	...
	#find local maxima---------------------------------
	ImgRes=64
	SHistAvg=np.histogram2d(Loc[SingleSpk,0],Loc[SingleSpk,1],bins=(np.arange(65),np.arange(65)))[0]
		...
		for j in range(i,2):
			if np.sqrt(i**2+j**2)<1.5:
				if j==0:
					...
					SHistAvgMax[i:,i:]+=SHistAvg[:-i,:-i]<SHistAvg[i:,i:]
					SHistAvgMax[:-i,:-i]+=SHistAvg[i:,i:]<SHistAvg[:-i,:-i]
					SHistAvgMax[i:,:-i]+=SHistAvg[:-i,i:]<SHistAvg[i:,:-i]
					SHistAvgMax[:-i,i:]+=SHistAvg[i:,:-i]<SHistAvg[:-i,i:]
				elif i==0:
	...
	XX=np.array(np.nonzero((SHistAvgMax>=3)*(SHistAvg.flatten()>np.percentile(SHistAvg,10)))[0],dtype=int)
	Freq=SHistAvg.flatten()[XX]
	Ncomp=len(XX)-Nignore
	print Ncomp
	XX=XX[np.argsort(np.argsort(np.abs(np.log(Freq*1./np.mean(Freq)))))<Ncomp]
	...
	#VSpikes=np.concatenate((np.ones(Spikes.shape[0],dtype=bool), np.zeros(NChannels*AddNoise,dtype=bool)))
	PSpikes=np.concatenate((np.zeros(Spikes.shape[0],dtype=bool), np.ones(NChannels*AddNoise,dtype=bool)))
	Spikes=np.concatenate((Spikes,\
	np.repeat(np.arange(NChannels),AddNoise)[np.argsort(scipy.rand(AddNoise*(NChannels)))]\
	...
	##np.array(scipy.rand(AddNoise*(NChannels))*Sampling*tMax,dtype=int),ClockSpikes))
	...
	for i in range(1,NChannels):
		SpikeHisto[i,:]=NSpikes[i]*NSpikesR*NextN*2./(TotSpikes-(NSpikes[i]*(SHistAvgMax[i]<>0)))#ok
	...
	for i in range(1,NChannels):
		...
		SpikeHisto[i,:]=NSpikes[i]*NSpikesR*NextN*2./(TotSpikes-(NSpikes[i]*(SHistAvgMax[i]<>0)))/fracGood
	...
	for k in range(len(probC)):
		for i in range(1,NChannels):
			b=np.arange(1,Ncomp+1)[(((a/ImgRes-i/ImgRes)**2+(a%ImgRes-i%ImgRes)**2)<ImgRes**2/2)\
			*(((a/ImgRes-i/ImgRes)**2+(a%ImgRes-i%ImgRes)**2)>1)]
			...
			(probC[k]*np.sqrt(np.clip(MeanS*MeanSR*1./NSpikes[i]*1./np.clip(NSpikesR[b],1,1e12),1e-12,10))))
			B=(scipy.stats.poisson.sf(X[i,b]+0.5,SpikeHistoTest)<\
			(probC[k]*np.sqrt(np.clip(MeanS*MeanSR*1./NSpikes[i]*1./np.clip(NSpikesR[b],1,1e12),1e-12,10))))*(X[i,b]>0)
	...
	for k in range(len(probC)):
		for i in range(1,NChannels):
			b=np.arange(1,Ncomp+1)[(((a/ImgRes-i/ImgRes)**2+(a%ImgRes-i%ImgRes)**2)<ImgRes**2/16)\
			*(((a/ImgRes-i/ImgRes)**2+(a%ImgRes-i%ImgRes)**2)>1)]
			SpikeHistoTest=(SpikeHisto[i,b]*(1.-(NZero[i]/2./NextN/(NSpikes[i]+(NSpikes[i]==0)))))
			SpikeHistoTest[SpikeHistoTest<=0.001]=0.001
			A=(scipy.stats.poisson.sf(X[i,b]-0.5,SpikeHistoTest)<\
			(0.25*probC[k]*np.sqrt(np.clip(MeanS*MeanSR*1./NSpikes[i]*1./np.clip(NSpikesR[b],1,1e12),1e-12,10))))
			B=(scipy.stats.poisson.sf(X[i,b]+0.5,SpikeHistoTest)<\
			(0.25*probC[k]*np.sqrt(np.clip(MeanS*MeanSR*1./NSpikes[i]*1./np.clip(NSpikesR[b],1,1e12),1e-12,10))))*(X[i,b]>0)
			Q=(scipy.stats.poisson.sf(X[i,b]-0.5,SpikeHistoTest)-\
			scipy.stats.poisson.sf(X[i,b]+0.5,SpikeHistoTest))[B-A]
			A[B-A]=scipy.rand(np.sum(B-A))<(Q/2.)
			NCorrX[i,b]=A
	...
	for i in range(UnitsXI[NextN],UnitsXI[-NextN]):
		...
		if Spikes[i]<>0:
			...
			a3=a1[(((a/ImgRes-XY[a1]/ImgRes)**2+(a%ImgRes-XY[a1]%ImgRes)**2)<64**2/16)\
			*(((a/ImgRes-XY[a1]/ImgRes)**2+(a%ImgRes-XY[a1]%ImgRes)**2)>1)]
			a4=a2[(((a/ImgRes-XY[a2]/ImgRes)**2+(a%ImgRes-XY[a2]%ImgRes)**2)<64**2/16)\
			*(((a/ImgRes-XY[a2]/ImgRes)**2+(a%ImgRes-XY[a2]%ImgRes)**2)>1)]
			a1=a1[(((a/ImgRes-XY[a1]/ImgRes)**2+(a%ImgRes-XY[a1]%ImgRes)**2)<64**2/2)\
			*(((a/ImgRes-XY[a1]/ImgRes)**2+(a%ImgRes-XY[a1]%ImgRes)**2)>1)]
			a2=a2[(((a/ImgRes-XY[a2]/ImgRes)**2+(a%ImgRes-XY[a2]%ImgRes)**2)<64**2/2)\
			*(((a/ImgRes-XY[a2]/ImgRes)**2+(a%ImgRes-XY[a2]%ImgRes)**2)>1)]
			...
			if len(a4)>4:
				...
				c=a8
				if len(b)>=5:
					c4=(np.sum(NCorrX[c,:][:,b])-len(c))*(1./len(a4))/np.clip(len(c),1,1e6)
			if len(a3)>4:
				b=a3[NCorrX[a,a3]]
				c=a7
				if len(b)>=5:
					c3=(np.sum(NCorrX[c,:][:,b])-len(c))*(1./len(a3))/np.clip(len(c),1,1e6)
			if len(a2)>4:
				b=a2[NCorr[a,a2]]
				c=a6
				if len(b)>=4:
...
def CorrelationAnalysis2(HdfFile):
	...
	for ii in range(1+IncludeLongSpikes):
		...
		for i in range(NChannels):
			if (NSpikes[i,ii])>50:
				PoissonIndx=PoissonInd[SpikesAll==i][SameSpikes[SpikesAll==i]]
				...
				N=ChC.shape[0]
				cP=CPoisson[SpikesPoisson==i].flatten()+1e-6*scipy.rand(np.sum(SpikesPoisson==i))
				aP=np.argsort(cP)
				#sort CI for putative events (where spikes are in distribution of Poisson events)
				...
				PmapX=np.arange(N)-np.cumsum(0.5*((ChAmp[aX]<=0.05)+(ChAmp[aY]<=0.05)))
				BiasC=np.clip(np.sqrt(np.linspace(2./N,2,N)*np.linspace(2,2./N,N)),0.1,1)
				AmpCiKink[i,ii]=np.mean(PmapX[np.argsort((X+Y-np.arange(1,N+1)*1./N)*1./BiasC)][:NS/20])*1./(NS-1)
				CINoise[i,ii]=1.-np.mean(np.clip((np.sort(iXS)*NS-np.arange(NS))\
				*1./(np.arange(1,NS+1)[::-1]),0,1))
				...
				fPoisson=NPoisson*1./NS
				nNextC=(NPoisson+NS)
				Cp=np.argsort(np.concatenate((CPoisson[SpikesPoisson==i].flatten()\
				,CS[(SpikesS==i)*LIndS[:,ii]].flatten())))<NPoisson#changed def
				Cpc=np.cumsum(np.concatenate((np.array([0]),Cp[:nNextC][::-1],Cp,Cp[-nNextC:][::-1])))
				...
				while jj<=1. and w:
					jj+=0.025
					nAll=int(nNextC*jj)
					pPoisson=(Cpc[Ind+nNextC+nAll+1]-Cpc[Ind+nNextC-nAll])/(2.*nAll+1.)+1e-6
					w=(np.max(pPoisson)*fNoise[i,ii])>1.1*np.mean(pPoisson)#should allow for being 10% off
				nS=int(jj*NS)
				pPoisson=(Cpc[Ind+nNextC+nAll+1]-Cpc[Ind+nNextC-nAll])/(2.*nAll+1.)+1e-6
				#print np.mean(pPoisson), fPoisson
				pPoisson=np.clip(pPoisson*fNoise[i,ii]*1./np.mean(pPoisson),0,1)#does not go well with clipping...
				pPoisson=np.clip(pPoisson*fNoise[i,ii]*1./np.mean(pPoisson),0,1)#do a round of adjustment to fNoise
				pPoisson=np.clip(pPoisson*fNoise[i,ii]*1./np.mean(pPoisson),0,1)#do a round of adjustment to fNoise
				fTP0=np.zeros(NS)
				fTP0[aXS]=1.-pPoisson
				...
				#print np.mean(pAmp)
				pAmp*=1./np.mean(pAmp)#does not go well with clipping... but shouldn't have that a large effect
